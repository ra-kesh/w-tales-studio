...
export function Expenses({ expenses }: ExpensesProps) {
  ...
  const groupedExpenses = expenses.reduce((acc, expense) => {
    ...
    const displayDate = expenseDate ? format(expenseDate, 'MMMM d, yyyy') : 'No Date';
  ...
  const sortedDates = Object.values(groupedExpenses).sort((a, b) => 
    b.dateTime.localeCompare(a.dateTime) // Reverse sort - newest first
  );
                  ...
                  <td className="py-5 text-right">
                    <div className="flex justify-end items-center">
                      ...
                      <span className="text-sm text-gray-500">
                        {expense.date 
                          ? format(new Date(expense.date as string), "MMM d, yyyy") 
                          : "No date"}
                      </span>
                    </div>
                  </td>
