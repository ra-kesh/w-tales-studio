...
import { receivedAmounts, bookings } from "@/lib/db/schema";
...
export async function GET(request: Request) {
	...
	try {
		...
		const [receivedAmountData, totalData] = await Promise.all([
			db
				.select({
					...
					description: receivedAmounts.description,
					paidOn: receivedAmounts.paidOn,
					createdAt: receivedAmounts.createdAt,
					updatedAt: receivedAmounts.updatedAt,
				})
